...
namespace DeckSwipe.CardModel.Import.Resource {
	...
	public class GoogleSheetsCollection {
		...
		private const int _majorFormatVersion = 1;
		...
		public async Task<ProtoCollection> Fetch() {
			...
			for (int i = 1; i < cardRowData.Length; i++) {
				...
				proto.rightAction.statsModification = new StatsModification(
					...
					cardRowData[i].values[14].IntValue);
		...
		private static bool CheckCardSheetFormat(Sheet sheet) {
			...
			if (headerRow.values[0].StringValue == "id"
					...
					&& headerRow.values[8].StringValue == "leftActionStat5"
					&& headerRow.values[9].StringValue == "rightActionText"
					&& headerRow.values[10].StringValue =="rightActionCoal" 
					&& headerRow.values[11].StringValue == "rightActionFood"
					&& headerRow.values[12].StringValue == "rightActionHealth"
					&& headerRow.values[13].StringValue == "rightActionHope"
					&& headerRow.values[14].StringValue == "rightActionStat5"
					&& headerRow.values[15].StringValue == "cardPrerequisites"
					&& headerRow.values[16].StringValue == "specialCardPrerequisites"
					&& headerRow.values[17].StringValue == "leftActionFollowupCardId"
					&& headerRow.values[18].StringValue == "leftActionFollowupCardDelay"
					&& headerRow.values[19].StringValue == "rightActionFollowupCardId"			
					&& headerRow.values[20].StringValue == "rightActionFollowupCardDelay") {
				return true;
			}
